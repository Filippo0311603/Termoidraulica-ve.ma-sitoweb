...
// LINK SOCIAL E WHATSAPP (Inserisci qui i tuoi link veri!)
const SOCIAL_LINKS = {
    facebook: "https://www.facebook.com/TUA_PAGINA",
    instagram: "https://www.instagram.com/TUA_PAGINA",
    whatsapp: "https://wa.me/393382611291" // Numero preso dal footer, formato internazionale senza +
};
...
// --- MODALE PREFERITI (Nuovo Componente) ---
...
const FavoritesModal = ({ isOpen, onClose, favorites, products, toggleFavorite }: FavoritesModalProps) => {
    ...
    // Filtra i prodotti completi basandosi sugli ID salvati nei preferiti
...
// --- PRODUCT MODAL (Aggiornato con tasto Preferiti) ---
...
// NAVBAR AGGIORNATA (Icona Preferiti)
        ...
        <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-lg py-3' : 'bg-transparent py-6'}`}>
            <div className="container mx-auto px-6 flex justify-between items-center">
                ...
                <div className="flex items-center gap-4 lg:hidden">
                    ...
                    <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className={`text-2xl ${scrolled ? 'text-gray-800' : 'text-white'}`}>
    ...
    return (
        <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
             <div className="absolute inset-0 z-0">
                <img src={HERO_IMAGE_URL} alt="Showroom Termoidraulica" className="w-full h-full object-cover" />
                ...
                <div className="max-w-2xl text-left pt-10">
                    ...
                    <div className="mt-16 flex items-center gap-10 text-white animate-slide-up delay-300 border-t border-white/10 pt-8">
                        ...
                        <div><h3 className="text-3xl font-bold text-white">10k+</h3><p className="text-xs text-gray-400 uppercase tracking-widest mt-1">Prodotti</p></div>
                         <div className="w-px h-12 bg-white/20"></div>
                        <div><h3 className="text-3xl font-bold text-white">4.9</h3><div className="flex text-orange-400 text-xs mt-1"><i className="fas fa-star"></i><i className="fas fa-star"></i><i className="fas fa-star"></i><i className="fas fa-star"></i><i className="fas fa-star"></i></div></div>
                    </div>
                </div>
...
// --- CATALOGO AGGIORNATO (Gestione Preferiti) ---
const Catalog = ({ products, favorites, toggleFavorite }: { products: Product[], favorites: (string | number)[], toggleFavorite: (id: string | number) => void }) => {
                ...
                <div className="flex flex-col lg:flex-row justify-between items-end mb-8 reveal gap-6">
                    ...
                    <div className="w-full lg:w-1/3 flex items-center gap-3">
                        <div className="relative w-full">
                            ...
                            <select value={activeCat} onChange={(e) => setActiveCat(e.target.value)} className="w-full appearance-none bg-white border border-gray-300 text-gray-700 py-3 pl-12 pr-10 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-500 cursor-pointer shadow-sm font-medium">
                    ...
                    {visibleProducts.length > 0 ? (
                        visibleProducts.map((product) => (
                            <div key={product.id} className="product-enter group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer relative" onClick={() => setSelectedProduct(product)}>
                                ...
                                <button 
                                    onClick={(e) => {
                                        e.stopPropagation(); // Evita di aprire il modale del prodotto
...
const Contact = () => {
    ...
    const sendEmail = (e: React.FormEvent) => {
        ...
        setIsSending(true);
        setFeedbackMsg("");
        // const SERVICE_ID = "TUO_ID";
        // const TEMPLATE_ID = "TUO_ID";
        // const PUBLIC_KEY = "TUA_KEY";
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    <div className="reveal">
                        ...
                        <div className="space-y-6 mb-10">
                            <div className="flex items-center gap-6 group"><div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-orange-500 text-2xl group-hover:bg-orange-500 group-hover:text-white transition-all duration-300"><i className="fas fa-map-marker-alt"></i></div><div><p className="text-sm text-gray-400 mb-1">Punto Vendita</p><p className="font-bold text-xl">Via delle Magnolie, 21, 00055 Ladispoli (RM)</p></div></div>
                            <div className="flex items-center gap-6 group"><div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-orange-500 text-2xl group-hover:bg-orange-500 group-hover:text-white transition-all duration-300"><i className="fas fa-phone-alt"></i></div><div><p className="text-sm text-gray-400 mb-1">Telefono</p><p className="font-bold text-xl">+39 338 261 1291</p></div></div>
                             <div className="flex items-center gap-6 group"><div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-orange-500 text-2xl group-hover:bg-orange-500 group-hover:text-white transition-all duration-300"><i className="fas fa-envelope"></i></div><div><p className="text-sm text-gray-400 mb-1">Email</p><p className="font-bold text-xl">max69vema@yahoo.it</p></div></div>
                        </div>
...
// --- FOOTER AGGIORNATO (Social e Link Iubenda) ---
const Footer = () => {
...
// --- COMPONENTE APP (Gestione Stato Globale) ---
const App = () => {
    ...
    const [favorites, setFavorites] = useState<(string | number)[]>(() => {
        const saved = localStorage.getItem('vema_favorites');
        return saved ? JSON.parse(saved) : [];
    });
    const [isFavoritesOpen, setIsFavoritesOpen] = useState(false);

    useEffect(() => {
        localStorage.setItem('vema_favorites', JSON.stringify(favorites));
    }, [favorites]);
    ...
    useEffect(() => {
        // 1. CARICAMENTO ISTANTANEO (File Lite)
        fetch('/products_lite.json')
            ...
            .then(liteData => {
                ...
                // 2. CARICAMENTO BACKGROUND (File Completo)
                fetch('/products.json')
                    .then(response => response.json())
                    .then(fullData => {
    ...
    return (
        <div className="font-sans text-gray-900 bg-slate-50 min-h-screen">
            ...
            {/* Passiamo i prodotti e i preferiti al Catalogo */}
            <Catalog products={products} favorites={favorites} toggleFavorite={toggleFavorite} />
